...
async function main() {
  try {
    ...
    const newList = user.createList('Yeni Calma Listem', true);
    const newList2 = user.createList('Yeni Calma Listem2', true);
    await db.listDatabase.save([newList]);
    await db.listDatabase.insert([newList2]);
    user.addToFavoriteLists(newList.id);
    const songAddedNewList = await user.addToList(newList.id, [song1.id, song2.id]);
    await db.listDatabase.update(songAddedNewList);

    await db.broadcastDatabase.save([user.createBroadcast('Prog Rock Yayını', false, true, [songs[0].id, songs[1].id])]);
